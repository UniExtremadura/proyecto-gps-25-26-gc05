...
const ArtistPage = () => {
    ...
    // --- LÓGICA NUEVA: FOTO DE PERFIL ---
    // Si tiene álbumes, cogemos la portada del primero. Si no, null.
        ...
        <div className="min-h-screen bg-black text-white font-sans pb-20">
            ...
            <div className="relative h-80 bg-gradient-to-b from-zinc-800 to-black flex items-end p-8">
                <div className="flex items-end gap-6 z-10">
                     ...
                     <div className="w-52 h-52 rounded-full overflow-hidden shadow-2xl bg-zinc-700 flex items-center justify-center border-4 border-black relative">
                        {artistImage ? (
                            <img 
                                ...
                                // Si la imagen falla, mostramos el icono de usuario
                                onError={(e) => {
                        ...
                        {/* Icono de respaldo (se muestra si no hay imagen o si falla la carga) */}
                 ...
                 {/* Fondo difuminado con la misma imagen para dar ambiente */}
                 {artistImage && (
                ...
                <section>
                    ...
                    <div className="bg-zinc-900/50 rounded-xl p-2">
                        {topTracks.length > 0 ? (
                            topTracks.map((track, index) => (
                                <div key={track.id} className="flex items-center p-3 hover:bg-white/10 rounded-md group transition-colors cursor-default">
                                    ...
                                    <div className="flex-1 px-4 font-medium text-white flex items-center gap-3">
                                        {/* Miniatura del álbum en la lista */}
                                        <img src={artistImage} className="w-10 h-10 rounded object-cover opacity-80" alt="" />
                                        {track.title}
                                    </div>
                            ...
                            <p className="text-zinc-500 italic p-4">No hay canciones populares disponibles.</p>
                        )}
                    </div>
                </section>
